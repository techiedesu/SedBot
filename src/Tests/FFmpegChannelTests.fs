...
let ``Reverse audio and video works properly`` () =
    task {
        ...
        | Result.Ok res ->
            ...
            match res with
            | Result.Ok _ -> Assert.That(File.Exists(resFile) && File.ReadAllBytes(resFile).Length > 0)
...
let ``No audio with reverse works properly`` () =
    task {
        let args =
            { FFmpeg.FFmpegObjectState.Create((new StreamReader("cb3fce1ba6ad45309515cbaf323ba18b.mp4")).BaseStream) with
                VideoReverse = true
                RemoveAudio = true }
