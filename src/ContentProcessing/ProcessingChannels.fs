                ...
                task {
                    ...
                    | FileType.Video ->
                        ...
                        | Result.Ok(_, distResultFileName) ->
                            let! res =
                                FFmpeg.appendAudioToVideoDistortion
                                    { VideoFileName = distResultFileName
                                      AudioFileName = inputFile }

                            return res
                        | Result.Error _ -> return res
                    | Voice ->
                    ...
                    | _ -> return! ImageMagick.convert { Src = stream; FileType = fileType }
                }
    ...
    let worker () =
        while cts.Task.IsCanceled |> not do
            try
                lambda().Wait()
            with ex ->
                logger.LogError("worker task crashed. restarting... ex: {ex}", ex)
