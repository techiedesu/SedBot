[<AutoOpen>]
module SedBot.Common.TypeExtensions
...
let inline swap ([<InlineIfLambda>] act: 'b -> 'a -> _) (a: 'a) (b: 'b) = act b a
...
let snackCaseToCamelCase (str: string) =
    ...
    [| for c in str do
           let c =
               if prev = '_' then Char.ToUpper c
               else if prev = '0' then Char.ToLower c
               else c
...
module String =
    let inline startsWith (text: string) (input: string) = text.StartsWith(input)
...
module Int32 =
    let inline tryParse (str: string) =
        str |> Int32.TryParse |> Option.ofCSharpTryPattern
...
[<RequireQualifiedAccess>]
module TaskVOption =
    let inline taskBind ([<InlineIfLambda>] binding) (v: 'v voption Task) =
        task {
            match! v with
...
module TaskOption =
    ...
    let inline map (a: 'a -> 'b) v : 'b option Task =
        task {
            let! v = v
...
module ActivePatterns =
    let (|Null|_|) obj =
        Object.ReferenceEquals(obj, null) |> Option.ofBool
