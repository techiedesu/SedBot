...
let internal createParseTree handleName : Parser<JsonValue, unit> =
    ...
    let tNumber =
        ...
        let numberParser =
            pipe2 sign number (fun (sign, int) frac -> $"{ofObj sign}{int}{toStr frac}")

        numberParser .>> skipSpacesAndNewLines |>> JsonValue.Number

    let tBool =
        (pstringCI "true" >>% true) <|> (pstringCI "false" >>% false)
    ...
    typeSelectorRef.Value <- tStr <|> tBool <|> tNull <|> tNumber <|> tObj <|> tArray

    skipSpacesAndNewLines >>. typeSelector .>> skipSpacesAndNewLines

let parse (input: string) (toCamelCase: bool) =
    let handleName = if toCamelCase then snackCaseToCamelCase else id

    match run (createParseTree handleName) input with
    | Success(result, _, _position) -> Some result
