...
module CommandPipelineItem =
    let set (item: CommandPipelineItem) command = item.SetCommand(command)

let rec public commandPatternInternal botUsername (text: string) chatType =
    let tryGetArgs rawArgs =
        let matched =
            Regex.Matches(rawArgs, """(?:(['"])(.*?)(?<!\\)(?>\\\\)*\1|([^\s]+))""")
    ...
    if text.StartsWith "t!" then
        ...
        | [ command ] -> Some(command.Substring(2).ToLowerInvariant()), None
        | head :: args -> Some(head.Substring(2).ToLowerInvariant()), tryGetArgs (args |> String.concat " ")
        | _ -> None, None
    elif chatType = SuperGroup then
        let command =
            Regex.Match(text, "(\/)(.*?)((@" + botUsername + " (\*.))|(@" + botUsername + "))")

        if command.Length > 0 then
            Some(
                command.Value
                    .Substring(1, command.Value.Length - botUsername.Length - 2)
                    .ToLowerInvariant()
            ),
            tryGetArgs (text.Substring(command.Value |> String.length))
        else
            None, None
    else
        ...
        if command.Success && command.Groups[2].Value = "" then
            Some(command.Value.Substring(1, command.Value.Length - 1)),
            tryGetArgs (text.Substring(command.Value.Length))
        else
...
let (|CommandWithArgs|) (item: CommandPipelineItem) =
    ...
    | { Text = Some text
        Chat = { Type = cType } } -> commandPatternInternal (item.BotUsername.Substring(1)) text cType
    | _ -> None, None
...
let (|Message|) (item: CommandPipelineItem) = item.Message

let (|Text|) (item: CommandPipelineItem) = item.Message.Text

let (|Caption|) (item: CommandPipelineItem) = item.Message.Caption
...
let (|ReplyMessage|) (item: CommandPipelineItem) = item.Message.ReplyToMessage
...
let (|WhenReplyDocumentMimeTypeIs|_|) (mimeType: string) (item: CommandPipelineItem) =
    match item with
    | ReplyMessage(Some { Document = Some { MimeType = Some mimeType' } }) -> Option.ofBool (mimeType = mimeType')
    | _ -> None
...
let (|ReplyPhotoMaxQualityId|) (item: CommandPipelineItem) =
    ...
    | ReplyPhoto(Some photos) ->
        let fileId =
            photos |> Array.sortBy _.Width |> Array.rev |> Array.head |> (fun p -> p.FileId)
...
let (|ChatId|) (item: CommandPipelineItem) =
    match item with
    | Message { Chat = { Id = id } } -> Some id

let (|MessageId|) (item: CommandPipelineItem) =
    match item with
    | Message { MessageId = id } -> Some id
...
let (|InlineHelp|_|) (item: CommandPipelineItem) = item.ProvideInlineHelp |> Option.ofBool
