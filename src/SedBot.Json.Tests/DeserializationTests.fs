...
type SmolUpdate = { UpdateId: int64; Chat: Chat option }
...
and NewChatMember = { Status: string; User: User voption }
...
let ``Smol object deserialization`` () =
    ...
    let deserialized =
        SedJsonDeserializer.deserialize typeof<MyChatMember> JsonSamples.SmolJson

    Assert.AreEqual(expected, deserialized)

    let deserialized =
        SedJsonDeserializer.deserializeStatic<MyChatMember> JsonSamples.SmolJson
...
let ``Nested option deserialization`` () =
    ...
    let deserialized =
        SedJsonDeserializer.deserializeStatic<Option<Option<Option<int>>>> "5"
...
let ``Complex object deserialization`` () =
    ...
    let res = SedJsonDeserializer.deserialize tUpdate JsonSamples.SampleUpdatesResponse
