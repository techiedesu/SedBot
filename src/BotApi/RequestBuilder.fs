...
let builderDynamic (t: Type) msg form =
    let reflectedFunc =
        typeof<Marker>.DeclaringType.GetMethod("buildExec").MakeGenericMethod(t)

    let build =
        Expression
            .Lambda<Func<MultipartFormDataContent -> IBotRequest -> string * StreamField list>>(
                Expression.Call(reflectedFunc)
            )
            .Compile()
            .Invoke()
...
let buildExec<'T when 'T :> IBotRequest> () =
    let invokedBuild form (msg: IBotRequest) = build<'T> form (msg :?> 'IBotRequest)
    invokedBuild
...
and mkPrinterCached<'T>
    (ctx: TypeGenerationContext)
    (form: MultipartFormDataContent option)
    (depth: int)
    : 'T -> string * StreamField list =
...
and mkPrinterAux<'T>
    ...
    (depth: int)
    : 'T -> string * StreamField list =
    let wrap (p: 'a -> string * StreamField list) =
        unbox<'T -> string * StreamField list> p
            ...
            { new ITypeVisitor<'T -> string * StreamField list> with
                member _.Visit<'a>() =
                    ...
                    wrap (fun ts ->
                        ts
                        |> List.fold
                            (fun (storedString: StringBuilder, storedStreams: StreamField list) v ->
        ...
        let mkUnionCasePrinter (unionCaseShape: ShapeFSharpUnionCase<'T>) =
            ...
            fun (u: 'T) ->
                ...
                | fps ->
                    fps
                    |> Seq.fold
                        (fun (printedStrAcc: StringBuilder, streamFieldsAcc: StreamField list) (_, fp) ->
                            ...
                            if printedStr = null then
                                ...
                                printedStrAcc.Append(printedStr).Append(","), streamFields)
                        ((StringBuilder(), []))
    ...
    | Shape.FSharpRecord(:? ShapeFSharpRecord<'T> as shape) ->
        ...
        fun (r: 'T) ->
            fieldPrinters
            |> Seq.fold
                (fun (storedString, storedStreams: StreamField list) (label, fieldPrinter) ->
                    ...
                    if streams.IsEmpty then
                        ...
                        form
                        |> Option.iter _.Add(new StreamContent(stream), camelCaseToSnakeCase label, fileName)

                        storedString, storedStreams)
                (([], []))
            |> fun (printedFields, streamFields) ->
                let printedFields = String.Join(", ", printedFields) in $"{{ {printedFields} }}", streamFields
    ...
    | Shape.Poco(:? ShapePoco<'T> as shape) ->
        ...
        fun (fieldType: 'T) ->
            propPrinters
            |> Seq.fold
                (fun (storedString: StringBuilder, storedStreams: StreamField list) (label, fieldPrinter) ->
