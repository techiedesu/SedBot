...
and Invoice =
    ...
    static member Create
        (title: string, description: string, startParameter: string, currency: string, totalAmount: int64)
        =
        { Title = title
          Description = description
...
and InlineKeyboardMarkup =
    ...
    static member Create(inlineKeyboard: InlineKeyboardButton[][]) = { InlineKeyboard = inlineKeyboard }
...
and VideoChatParticipantsInvited =
    ...
    static member Create(users: User[]) = { Users = users }
...
and VideoNote =
    ...
    static member Create
        (fileId: string, fileUniqueId: string, length: int64, duration: int64, ?thumbnail: PhotoSize, ?fileSize: int64) =
        { FileId = fileId
          FileUniqueId = fileUniqueId
          Length = length
          Duration = duration
...
and VideoChatEnded =
    ...
    static member Create(duration: int64) = { Duration = duration }
...
and VideoChatScheduled =
    ...
    static member Create(startDate: int64) = { StartDate = startDate }
...
and Dice =
    ...
    static member Create(emoji: string, value: int64) = { Emoji = emoji; Value = value }
...
and InlineQuery =
    ...
    static member Create
        (id: string, from: User, query: string, offset: string, ?chatType: ChatType, ?location: Location)
        =
        { Id = id
          From = from
          Query = query
...
and ChatJoinRequest =
    ...
    static member Create
        (chat: Chat, from: User, userChatId: int64, date: DateTime, ?bio: string, ?inviteLink: ChatInviteLink)
        =
        { Chat = chat
          From = from
...
and PollOption =
    ...
    static member Create(text: string, voterCount: int64) =
        { Text = text; VoterCount = voterCount }
...
and ShippingAddress =
    ...
    static member Create
        (countryCode: string, state: string, city: string, streetLine1: string, streetLine2: string, postCode: string) =
...
and ChatMemberLeft =
    ...
    static member Create(status: string, user: User) = { Status = status; User = user }
